...
int pe_output_buffer(TCCState * s1, byte **out_buffer);
...
void tcc_set_header_file(TCCState *s1, char *fullPath)
{
	//tcc_add_symbol(s1, "__tcc_fpu_control", (void *)&__tcc_fpu_control);
	//tcc_add_symbol(s1, "__tcc_int_fpu_control", (void *)&__tcc_int_fpu_control);

	strcpy(headerFile, fullPath);
}
...
static int tcc_compile(TCCState *s1)
{
	...
	//add_elf_sym(symtab_section, (unsigned long)&__tcc_fpu_control, 2, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, SHN_ABS, "__tcc_fpu_control");
	//add_elf_sym(symtab_section, (unsigned long)&__tcc_int_fpu_control, 2, ELFW(ST_INFO)(STB_GLOBAL, STT_NOTYPE), 0, SHN_ABS, "__tcc_int_fpu_control");

	f = tcc_open(s1, headerFile);
    *s1->include_stack_ptr++ = file;
    f->inc_type = '\"';
    pstrcpy(f->inc_filename, sizeof(f->inc_filename), headerFile);
    file = f;
