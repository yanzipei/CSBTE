...
void Mod_LoadStudioModel(model_t *mod, byte *buffer)
{
	...
	if (!gConfigs.bEnableCSOModelLoad)
		return g_pfnMod_LoadStudioModel(mod, buffer);
	...
	for (int i = 0; i < studiohdr->numtextures; i++)
	{
		...
		{
			int cwidth, cheight;
			sscanf(&ptexture[i].name[1], "%3d%3d", &cwidth, &cheight);

			ptexture[i].width = cwidth;
			ptexture[i].height = cheight;
		}
	}
	...
	if (debugTime)
		gEngfuncs.Con_Printf("Mod_LoadStudioModel: load %s take %.4f sec.\n", mod->name, gPerformanceCounter.GetCurTime() - startTime);
}
