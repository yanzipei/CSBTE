...
__declspec(naked) int V_UTF8ToUChar32(const char *pUTF8, wchar_t &uValueOut, bool &bErrorOut)
{
	_asm
	{
		push ebp
...
bool V_UnicodeValidate(const char *pUTF8)
{
	bool bError = false;
	static wchar_t uVal;
	int nCharSize;
	...
	while (*pUTF8)
	{
		nCharSize = V_UTF8ToUChar32(pUTF8, uVal, bError);
