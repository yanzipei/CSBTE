...
void Frame::OnFrameFocusChanged(bool bHasFocus)
{
	...
	if (bHasFocus)
	{
		if (m_flFocusTransitionEffectTime)
		{
			...
			if (m_iOutOfFocusAlpha != -1)
				GetAnimationController()->RunAnimationCommand(this, "Alpha", 255, 0.0f, m_flTransitionEffectTime, AnimationController::INTERPOLATOR_LINEAR);
		}
		else
		{
			if (m_iOutOfFocusAlpha != -1)
	...
	else
	{
		if (m_flFocusTransitionEffectTime)
		{
			...
			if (m_iOutOfFocusAlpha != -1 && m_bFadingOut != true)
				GetAnimationController()->RunAnimationCommand(this, "Alpha", m_iOutOfFocusAlpha, 0.0f, m_flTransitionEffectTime, AnimationController::INTERPOLATOR_LINEAR);
		}
		else
		{
			if (m_iOutOfFocusAlpha != -1)
				SetAlpha(m_iOutOfFocusAlpha);
...
//-----------------------------------------------------------------------------
// Purpose: Check the closeing of the panel
//-----------------------------------------------------------------------------
bool Frame::IsCloseing()
{
	return m_bCloseing;
}
...
//-----------------------------------------------------------------------------
void Frame::DrawBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha, bool hollow /*=false*/ )
{
	if (!m_bImageBackground)
		return BaseClass::DrawBox(x, y, wide, tall, color, normalizedAlpha, hollow);
...
void Frame::ApplySchemeSettings(IScheme *pScheme)
{
	...
	resourceString = pScheme->GetResourceString("Frame.OutOfFocusAlpha");

	if (resourceString[0])
		m_iOutOfFocusAlpha = atoi(resourceString);
}
...
//-----------------------------------------------------------------------------
bool Frame::LoadControlSettings( const char *dialogResourceName, const char *pathID, KeyValues *pPreloadedKeyValues )
{
	bool bSuccess = BaseClass::LoadControlSettings( dialogResourceName, pathID, pPreloadedKeyValues );
