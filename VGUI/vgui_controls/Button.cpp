...
// Purpose: 
//-----------------------------------------------------------------------------
Button::ActivationType_t Button::GetButtonActivationType(ActivationType_t activationType)
{
...
//-----------------------------------------------------------------------------
void Button::DrawBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha, bool hollow /*=false*/ )
{
	if ( !ShouldPaint() )
		return; 
...
void Button::ApplySchemeSettings(IScheme *pScheme)
{
	...
	resourceString = pScheme->GetResourceString("Button.SelectedInsetX");
	...
	resourceString = pScheme->GetResourceString("Button.SelectedInsetY");
	...
	SetSelectedInset(xInset, yInset);
	...
	if ( _buttonFlags.IsFlagSet( DEFAULT_BUTTON ) != (bool)state )
	{
		...
		if (state)
		{
			...
			if (GetVParent())
			{
				...
				HPanel handle = ToHandle();
				Assert( handle );
				msg->SetInt("button", handle);
