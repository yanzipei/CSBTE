...
void Label::SetSelectedInset(int xInset, int yInset)
{
	_selectedInset[0] = xInset;
	_selectedInset[1] = yInset;
}
	...
	for (int i = 0; i < _imageDar.Count(); i++)
	{
		...
		if (image == _textImage)
		{
			if (IsEnabled())
			{
				...
				{
					_textImage->SetPos(x + _selectedInset[0], imageYPos + _selectedInset[1]);
				}
...
//-----------------------------------------------------------------------------
void Label::SetImageBounds(int index, int x, int width, int y)
{
	_imageDar[index].xpos = (short)x;
	_imageDar[index].ypos = (short)y;
