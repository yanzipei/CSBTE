...
public:
	...
	virtual void Paint()
	{
		if (_CheckButton->_imageBackground)
		{
			int imageWide, imageTall;

			if (_CheckButton->IsSelected())
			{
				_CheckButton->_onImage->GetSize(imageWide, imageTall);
				_CheckButton->_onImage->SetPos(0, (_CheckButton->GetTall() - imageTall) / 2);
				_CheckButton->_onImage->Paint();
			}
			else
			{
				_CheckButton->_offImage->GetSize(imageWide, imageTall);
				_CheckButton->_offImage->SetPos(0, (_CheckButton->GetTall() - imageTall) / 2);
				_CheckButton->_offImage->Paint();
			}
...
void CheckButton::ApplySchemeSettings(IScheme *pScheme)
{
	...
	int xInset, yInset;
	GetTextInsert(xInset, yInset);

	const char *resourceString = pScheme->GetResourceString("CheckButton.TextInsetX");

	if (resourceString[0])
		xInset = atoi(resourceString);

	resourceString = pScheme->GetResourceString("CheckButton.TextInsetY");
	...
	resourceString = pScheme->GetResourceString("CheckButton.On");

	if (resourceString[0])
	{
		_imageBackground = true;
		_onImage = scheme()->GetImage(resourceString , true);
		_offImage = scheme()->GetImage(pScheme->GetResourceString("CheckButton.Off"), true);
	}
}
