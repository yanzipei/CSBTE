...
public:
	...
	virtual void Paint()
	{
		if (_radioButton->_imageBackground)
		{
			int imageWide, imageTall;

			if (_radioButton->IsSelected())
			{
				_radioButton->_onImage->GetSize(imageWide, imageTall);
				_radioButton->_onImage->SetPos(0, (_radioButton->GetTall() - imageTall) / 2);
				_radioButton->_onImage->Paint();
			}
			else
			{
				_radioButton->_offImage->GetSize(imageWide, imageTall);
				_radioButton->_offImage->SetPos(0, (_radioButton->GetTall() - imageTall) / 2);
				_radioButton->_offImage->Paint();
			}
...
void RadioButton::ApplySchemeSettings(IScheme *pScheme)
{
	...
	const char *resourceString = pScheme->GetResourceString("RadioButton.On");

	if (resourceString[0])
	{
		_imageBackground = true;
		_onImage = scheme()->GetImage(resourceString , true);
		_offImage = scheme()->GetImage(pScheme->GetResourceString("RadioButton.Off"), true);
	}
}
