...
public:
	...
	void DrawBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha, bool hollow /*=false*/ )
	{
		if ( !ShouldPaint() )
			return; 
...
//-----------------------------------------------------------------------------
void PropertySheet::DrawBox(int x, int y, int wide, int tall, Color color, float normalizedAlpha, bool hollow /*=false*/ )
{
...
void PropertySheet::OnCurrentDefaultButtonSet( vgui::VPANEL defaultButton )
{
	...
	if (GetVParent())
	{
		...
		HPanel handle = ivgui()->PanelToHandle( defaultButton );
		Assert( handle );
