...
void Panel::ApplySettings(KeyValues *inResourceData)
{
	...
	if ( wstr )
	{
		...
		{
			wide = -wide;
			_buildModeFlags |= BUILDMODE_SAVE_DIRECT_WIDE;
		}
	...
	{
		tall = -tall;
		_buildModeFlags |= BUILDMODE_SAVE_DIRECT_TALL;
	}
...
void Panel::GetSettings( KeyValues *outResourceData )
{
	...
	else if (_buildModeFlags & BUILDMODE_SAVE_DIRECT_WIDE )
	{
	...
	if (_buildModeFlags & BUILDMODE_SAVE_DIRECT_TALL )
	{
