...
public:
	...
	virtual void SetSelectedInset(int xInset, int yInset);

	void GetTextInsert(int &xInset, int &yInset)
	{
		xInset = _textInset[0];
		yInset = _textInset[1];
	}
	...
	virtual void SetImageBounds(int index, int x, int width, int y = -1);
	...
	*/
	...
	void SetOffsetSelectEnabled( bool state );
