...
void WizardPanel::OnNextButton()
{
	if (_currentSubPanel)
	{
		...
		if (shouldAdvance)
		{
			...
			if (nextPanel)
			{
				ActivateNextSubPanel(nextPanel);
			}
		}
	}
}
...
//-----------------------------------------------------------------------------
// Purpose: whether to enable the next,prev,finish and cancel buttons
//-----------------------------------------------------------------------------
void WizardPanel::EnableButtons(bool state)
{ 
	_showButtons = state; 	// hide the wizard panel buttons
	SetNextButtonEnabled( state );
	SetPrevButtonEnabled( state );
	SetFinishButtonEnabled( state );
	SetCancelButtonEnabled( state );
}
