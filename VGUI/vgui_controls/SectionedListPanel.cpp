...
public:
	...
	{
		m_bSelected = false;
		m_bOverrideFgColors = false;
		m_bOverrideBgColors = false;
		m_iSectionID = -1;
	...
	virtual void SetOverrideFgColors( bool state )
	{
		m_bOverrideFgColors = state;
	}

	virtual void SetOverrideBgColors( bool state )
	{
		m_bOverrideBgColors = state;
	}
...
private:
	...
	bool m_bSelected;
	bool m_bOverrideFgColors;
	bool m_bOverrideBgColors;
	bool m_bShowColumns;
};
...
// Purpose: 
//-----------------------------------------------------------------------------
void SectionedListPanel::PaintBackground()
{
...
void SectionedListPanel::LayoutPanels(int &contentTall)
{
	...
	int x = m_iPaintOffset[0], wide = GetWide() - 10;
	int y = m_iPaintOffset[1];
...
void SectionedListPanel::ApplySettings(KeyValues *inResourceData)
{
	...
	if( !inResourceData->GetInt("sectionheader", 1 ) ) // if "section header" is "0" then don't draw the section header
	{
		SetSectionHeaderVisible( false );
	}
...
void SectionedListPanel::SetItemFgColor( int itemID, Color color )
{
	...
	m_Items[itemID]->SetOverrideFgColors( true );
	m_Items[itemID]->InvalidateLayout();
}
...
void SectionedListPanel::SetItemBgColor( int itemID, Color color )
{
	...
	m_Items[itemID]->SetOverrideBgColors( true );
	m_Items[itemID]->InvalidateLayout();
}
...
bool SectionedListPanel::IsSectionAlwaysVisible(int sectionID)
{
	if (!m_Sections.IsValidIndex(sectionID))
		return false;

	return m_Sections[sectionID].m_bAlwaysVisible;
}
...
void SectionedListPanel::SetPaintOffset(int x, int y)
{
	m_iPaintOffset[0] = x;
	m_iPaintOffset[1] = y;
}

void SectionedListPanel::SetSectionHeaderVisible(bool state)
{
	m_bHeaderVisible = state;
}

bool SectionedListPanel::IsSectionHeaderVisible(void)
{
