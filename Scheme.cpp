...
static const char *s_pszSearchString = NULL;
CUtlRBTree<CachedBitmapHandle_t, int> m_Bitmaps;
bool m_bAlreadyShutdown;

class CSchemeManager_S
{
public:
	CSchemeManager_S(void);
	~CSchemeManager_S(void);
};

CSchemeManager_S g_Scheme_S;
...
CSchemeManager_S::CSchemeManager_S(void)
{
	CScheme *nullScheme = new CScheme();
	m_Schemes.AddToTail(nullScheme);
	m_Bitmaps.SetLessFunc(&CSchemeManager::BitmapHandleSearchFunc);
	m_bAlreadyShutdown = false;
}
...
HScheme CSchemeManager::LoadSchemeFromFileEx(VPANEL sizingPanel, const char *fileName, const char *tag)
{
	...
	DevMsg("CSchemeManager::LoadSchemeFromFileEx(%s) take %.1f sec\n", fileName, gPerformanceCounter.GetCurTime() - startTime);
...
HScheme CSchemeManager::LoadSchemeFromFile(const char *fileName, const char *tag)
{
	if (tag)
	{
		...
		{
			sprintf(szPath, "resource/scheme/%s/TrackerScheme.res", gConfigs.szUIScheme);

			if (LoadSchemeFromFileEx(0, szPath, tag))
				return TRUE;
		}
		else if (!strcmp(tag, "ClientScheme"))
		{
			sprintf(szPath, "resource/scheme/%s/ClientScheme.res", gConfigs.szUIScheme);
...
void CScheme::ReloadFontGlyphs(void)
{
	...
	for (int i = 0; i < m_FontAliases.Count(); i++)
	{
		...
		for (KeyValues *fontdata = kv->GetFirstSubKey(); fontdata != NULL; fontdata = fontdata->GetNextKey())
		{
			...
			if (fontdata->GetInt("freetype"))
				flags |= ISurface::FONTFLAG_FREETYPE;

			if (fontdata->GetInt("unlimited"))
				flags |= ISurface::FONTFLAG_UNLIMITED;
...
int CSchemeManager::GetProportionalScaledValue_(int rootWide, int rootTall, int normalizedValue)
{
	...
	return (int)((normalizedValue * scale) + 0.5);
}
...
int CSchemeManager::GetProportionalNormalizedValue_(int rootWide, int rootTall, int scaledValue)
{
	...
	double scale = (double)rootTall / (double)proH;
	return (int)((scaledValue / scale) + 0.5);
}
...
{
	char *language = gConfigs.szLanguage;
...
{
	DWORD *pVFTable = *(DWORD **)&g_Scheme;

	g_pScheme = pSchemeManager;
	g_pMetaHookAPI->VFTHook(g_pScheme, 0, 1, (void *)pVFTable[1], (void *&)g_pfnCSchemeManager_LoadSchemeFromFile);
	g_pMetaHookAPI->VFTHook(g_pScheme, 0, 2, (void *)pVFTable[2], (void *&)g_pfnCSchemeManager_ReloadSchemes);
	g_pMetaHookAPI->VFTHook(g_pScheme, 0, 3, (void *)pVFTable[3], (void *&)g_pfnCSchemeManager_GetDefaultScheme);
	g_pMetaHookAPI->VFTHook(g_pScheme, 0, 4, (void *)pVFTable[4], (void *&)g_pfnCSchemeManager_GetScheme);
	g_pMetaHookAPI->VFTHook(g_pScheme, 0, 5, (void *)pVFTable[5], (void *&)g_pfnCSchemeManager_GetImage);
	g_pMetaHookAPI->VFTHook(g_pScheme, 0, 6, (void *)pVFTable[6], (void *&)g_pfnCSchemeManager_GetImageID);
	g_pMetaHookAPI->VFTHook(g_pScheme, 0, 7, (void *)pVFTable[7], (void *&)g_pfnCSchemeManager_GetIScheme);
	g_pMetaHookAPI->VFTHook(g_pScheme, 0, 8, (void *)pVFTable[8], (void *&)g_pfnCSchemeManager_Shutdown);
	g_pMetaHookAPI->VFTHook(g_pScheme, 0, 9, (void *)pVFTable[9], (void *&)g_pfnCSchemeManager_GetProportionalScaledValue);
	g_pMetaHookAPI->VFTHook(g_pScheme, 0, 10, (void *)pVFTable[10], (void *&)g_pfnCSchemeManager_GetProportionalNormalizedValue);
}
...
const char *ISchemeManager::GetSchemeName(const char *moduleName)
{
	return gConfigs.szUIScheme;
}
