...
void IInput::SetFocus(VPANEL panel)
{
	g_Input.SetFocus(panel);
}
...
void CInput::InternalKeyTyped(wchar_t unichar)
{
	...
	int imeLang = GetCurrentIMELanguage();

	if (imeLang == 0x042f)
		MultiByteToWideChar(1251, 0, &cChar, -1, &unichar, 1);
...
LRESULT ImeWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		...
		{
			g_pInput->OnIMEComposition(lParam);
			return 0;
		}
...
{
	((CInput *)this)->GetIMELanguageName(buf, unicodeBufferSizeInBytes);
}
...
{
	((CInput *)this)->SetIMEWindow(hwnd);
}

void IInput::SetIMEEnabled(bool state)
{
	((CInput *)this)->SetIMEEnabled(state);
}

void IInput::UpdateCursorPosInternal(int x, int y)
{
	Assert(0);
}

void IInput::PostCursorMessage(void)
{
	Assert(0);
}

void IInput::HandleExplicitSetCursor(void)
{
	Assert(0);
}