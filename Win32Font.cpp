...
bool CWin32Font::Create(const char *windowsFontName, int tall, int weight, int blur, int scanlines, int flags)
{
	...
	if (!strnicmp(&windowsFontName[len - 4], ".tt", 3))
	{
		...
		{
			const char *faceName = FontManager().GetFontFaceName(fullPath);

			if (faceName)
			{
				FontManager().AddCustomFontFile(fullPath);

				m_hFont = ::CreateFontA(tall, 0, 0, 0, m_iWeight, flags & vgui::ISurface::FONTFLAG_ITALIC, flags & vgui::ISurface::FONTFLAG_UNDERLINE, flags & vgui::ISurface::FONTFLAG_STRIKEOUT, charset, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, m_bAntiAliased ? ANTIALIASED_QUALITY : NONANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, faceName);
			}
		}
