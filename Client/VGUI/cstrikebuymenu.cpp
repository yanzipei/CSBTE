...
{
	m_pMainMenu->LoadControlSettingsFromScheme("UI/BuyMenu_CT.res", "GAME");
...
{
	m_pMainMenu->LoadControlSettingsFromScheme("UI/BuyMenu_TER.res", "GAME");
	m_pMainMenu->SetVisible(false);
...
CCSBaseBuyMenu::CCSBaseBuyMenu(const char *subPanelName) : CBuyMenu()
{
	...
	LoadControlSettingsFromScheme("UI/BuyMenu.res", "GAME");
...
void CCSBuySubMenu::SetupNextSubPanel(const char *fileName)
{
	...
	if (i == m_SubMenus.Count())
	{
		...
		newMenu->LoadControlSettingsFromScheme(fileName, "GAME");
		m_NextPanel = newMenu;
