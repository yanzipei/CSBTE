...
#define PLAYER_FATAL_FALL_SPEED (float)1024
#define PLAYER_MAX_SAFE_FALL_SPEED (float)580
...
#define PLAYER_LONGJUMP_SPEED (float)350
...
void PM_InitTextureTypes(void)
{
	...
	pMemFile = pmove->COM_LoadFile("sound/materials.txt", 5, 0);
	...
	while (pmove->memfgets(pMemFile, fileSize, &filePos, buffer, sizeof(buffer) - 1) && gcTextures < CTEXTURESMAX)
	{
		i = 0;
...
void PM_UpdateStepSound(void)
{
	...
	if (pmove->flags & FL_DUCKING || fLadder)
	{
		...
		if ((pmove->flags & FL_DUCKING) && pmove->flDuckTime < 950)
			fvol *= 0.35;
	}
		...
		if (pmove->movetype == MOVETYPE_WALK && (pmove->onground == -1 || pmove->friction != 1))
		{
			for (i = 0; i < numplanes; i++)
			{
				if (planes[i][2] > 0.7)
				{
					...
					PM_ClipVelocity(original_velocity, planes[i], new_velocity, 1 + pmove->movevars->bounce * (1 - pmove->friction));
			}
...
void PM_WalkMove(void)
{
	...
	{
		float factor;
		factor = (100.0 - pmove->fuser2 * 0.001 * 19) * 0.01;
...
void PM_Friction(void)
{
	...
	vel = pmove->velocity;
	speed = sqrt(pmove->velocity[0] * pmove->velocity[0] + pmove->velocity[1] * pmove->velocity[1] + pmove->velocity[2] * pmove->velocity[2]);
...
qboolean PM_CheckWater(void)
{
	...
	if (cont <= CONTENTS_WATER && cont > CONTENTS_TRANSLUCENT)
	{
		...
		{
			static vec3_t current_table[] = { { 1, 0, 0 }, { 0, 1, 0 }, { -1, 0, 0 }, { 0, -1, 0 }, { 0, 0, 1 }, { 0, 0, -1 } };
			VectorMA(pmove->basevelocity, pmove->waterlevel * 50.0, current_table[CONTENTS_CURRENT_0 - truecont], pmove->basevelocity);
		}
	}
...
int PM_CheckStuck(void)
{
	...
	if (!pmove->server && (hitent == 0 || pmove->physents[hitent].model != NULL))
	{
		int nReps = 0;
		PM_ResetStuckOffsets(pmove->player_index, pmove->server);
...
void PM_Duck(void)
{
	...
	int buttonsChanged = pmove->cmd.buttons ^ pmove->oldbuttons;
	...
	if (pmove->cmd.buttons & IN_DUCK)
	{
		...
		if (pmove->bInDuck)
		{
			if (pmove->flDuckTime / 1000.0 <= (1 - TIME_TO_DUCK) || pmove->onground == -1)
			{
				pmove->usehull = 1;
...
void PM_LadderMove(physent_t *pLadder)
{
	...
	if (trace.fraction != 1)
	{
		...
		vec3_t vpn, v_right;
		float flSpeed = pmove->maxspeed;

		if (pmove->maxspeed > MAX_CLIMB_SPEED)
			flSpeed = MAX_CLIMB_SPEED;
...
void PM_Physics_Toss(void)
{
	...
	if (trace.plane.normal[2] > 0.7)
	{
		...
		{
			VectorScale(pmove->velocity, (1 - trace.fraction) * pmove->frametime * 0.9, move);
			trace = PM_PushEntity(move);
		}
...
void PM_Jump(void)
{
	...
	if (pmove->waterlevel >= 2)
	{
		...
		if (pmove->flSwimTime <= 0)
		{
			...
			{
				case 0: pmove->PM_PlaySound(CHAN_BODY, "player/pl_wade1.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
				case 1: pmove->PM_PlaySound(CHAN_BODY, "player/pl_wade2.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
				case 2: pmove->PM_PlaySound(CHAN_BODY, "player/pl_wade3.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
				case 3: pmove->PM_PlaySound(CHAN_BODY, "player/pl_wade4.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
			}
		}
	...
	if (~pmove->oldbuttons & IN_JUMP && (!pmove->bInDuck || ~pmove->flags & FL_DUCKING))
	{
		...
		if (pmove->fuser2 > 0)
			pmove->velocity[2] *= (100.0 - pmove->fuser2 * 0.001 * 19) * 0.01;
...
void PM_CheckWaterJump(void)
{
	...
	if (tr.fraction < 1.0 && fabs(tr.plane.normal[2]) < 0.1)
	{
...
{
	if ((!pmove->oldwaterlevel && pmove->waterlevel) || (pmove->oldwaterlevel && !pmove->waterlevel))
	{
		switch (pmove->RandomLong(0, 3))
		{
			case 0: pmove->PM_PlaySound(CHAN_BODY, "player/pl_wade1.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
...
void PM_DropPunchAngle(vec3_t punchangle)
{
	...
	len -= (10 + len * 0.5) * pmove->frametime;
	len = max(len, 0);
...
void PM_CheckParamters(void)
{
	...
	if (pmove->angles[YAW] > 180)
		pmove->angles[YAW] -= 360;
}
...
void PM_PlayerMove(qboolean server)
{
	...
	{
		default: pmove->Con_DPrintf("Bogus pmove player movetype %i on (%i) 0=cl 1=sv\n", pmove->movetype, pmove->server); break;

		case MOVETYPE_NONE: break;
		case MOVETYPE_NOCLIP: PM_NoClip(); break;
		case MOVETYPE_TOSS:
		case MOVETYPE_BOUNCE: PM_Physics_Toss(); break;
